...
with tab1:
    ...
    ## st.write(data) ## used only for debugging purpose
    ...
    choice = st.sidebar.multiselect(
        "Pick Airlines",
        ('US Airways', 'United', 'Delta', 'American'),
        key=4
    )
    ...
    ## Facets = splitting one big chart into multiple small charts
...
with tab2:
    ...
    st.write("Ask questions about airline sentiment data")
    ...
    if not api_key:
        ...
        if user_input:
            with st.spinner("Thinking..."):
                response = get_chatbot_response(api_key, user_input, data)
                st.write(response)
